...
import { globalI18nMap } from "./extract.js";
import { defaultOptions } from "./options.js";

export default async function (source) {
  // 获取 Webpack 的 Loader 上下文，方便访问文件路径及其他相关信息
  const global = this;
  const option = { ...defaultOptions, ...global.getOptions() };
  const {
  ...
  try {
    ...
    if (
      ...
      !global.resourcePath.includes(resolveAliasPath(importPath))
    ) {
      source = await i18nImportTransform(
        source,
        global.resourcePath,
        jsx
