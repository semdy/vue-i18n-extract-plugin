...
function getLangJsonPath(langKey, option) {
  const path = relativeCWDPath(option.outputPath + langKey + ".json");
  return path;
}
  ...
  Object.keys(toTransLangObjMap).forEach(langKey => {
    if (!isEmptyObject(toTransLangObjMap[langKey])) {
      try {
        ...
        // ─── 写入文件系统 ───
        fs.outputJson(getLangJsonPath(langKey, option), mergedLangObj, {
          spaces: 2
        });
...
async function translateChunks(transLangObj, toTranslateLang, option) {
  ...
  const handleTranslatedText =
    typeof option.handleTranslatedText !== "function"
      ? text => text
    ...
    .map(item => {
      ...
      const splitTranslation = (text, separatorRegex) => {
        return text
          ...
          .map(v => handleTranslatedText(v.trim(), toTranslateLang));
      };
