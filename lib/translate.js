  ...
  const commitBuffer = () => {
    if (buffer.length > 0) {
      ...
      if (actualLength > maxChunkSize) {
        console.warn(
          `⚠️ buffer commit exception: actual length ${actualLength} exceeds limit`
        );
      }
  ...
  for (const value of values) {
    ...
    if (value.length > maxChunkSize) {
      ...
      if (value.length > maxChunkSize * 1.5) {
        console.warn(
          `⚠️ Long text warning: Detected a text item with ${value.length} characters, which may affect translation quality.`
        );
      }
      // 结果直接新增一个超长文本
      result.push(value);
...
async function autoTranslateFromFile() {
  ...
  if (!fromLangMap) {
    console.warn(
      `⚠️ The source language file ${defaultOptions.fromLang}.json is empty. Please add source language content first.`
    );
    return;
  }
...
async function autoTranslate(i18nMap, option) {
  ...
  for (const langKey of Object.keys(toTransLangObjMap)) {
    ...
    if (curLangKeys.length === 0) {
      if (option.debug) {
        console.info(`👉 ${langKey} don't need translation, skipping...`);
      }
      continue;
    }
    ...
    if (option.debug) {
      console.info(
        `🔄 [${option.translator.option.name}] start translate ${langKey}`
      );
    }
    ...
    if (translatedValues.length !== curLangKeys.length) {
      console.error(
        "❌ When using paid translation services, please check whether your translation API quota is sufficient or whether you have applied for the required translation API access permissions."
      );
      console.error(`❌ ${langKey} translation result is incomplete
        Expected count: ${curLangKeys.length}
        Actual count: ${translatedValues.length}
        Sample data: ${JSON.stringify(translatedValues.slice(0, 3))}`);
      // return;
    }
    ...
    if (option.debug) {
      console.info(`🎉 ${langKey} translated`);
    }
  ...
  for (const langKey of Object.keys(toTransLangObjMap)) {
    if (!isEmptyObject(toTransLangObjMap[langKey])) {
      try {
        ...
        console.info(`🎉 ${langKey} file updated`);
      } catch (error) {
        console.error(`❌ ${langKey} file write failed, reason:`, error);
      }
    }
  }
}
...
async function cleanTranslate(option) {
  ...
  for (const langKey of Object.keys(langObjMap)) {
    ...
    console.info(`🎉 ${langKey} file cleaned`);
  }
}
