    ...
    createI18nVisitor(
      {
        ...
        keepDefaultMsg: options.keepRaw ? false : options.keepRaw
      },
      i18nMap
    )
  );
...
function transformSvelteTemplate(htmlAst, options, ms) {
  ...
  function walk(node) {
    ...
    if (node.type === "Text") {
      ...
      if (text) {
        const replacement = handleI18n(text, options);
      ...
      node.value.forEach(v => {
        if (v.type === "Text") {
          ...
          if (text) {
            ...
            if (replacement !== text) {
              changed = true;
              ms.overwrite(v.start, v.end, `"${replacement}"`);
            }
          }
        }
      });
...
function processSvelteFile(code, options) {
  ...
  // 处理 <script>
  if (ast.instance) {
    const scriptResult = transformScript(ast.instance.content, options);
  ...
  if (ast.module) {
    const moduleResult = transformScript(ast.module.content, options);
