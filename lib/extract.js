...
function transformMarkoTemplate(code, options, ms) {
  ...
  const parser = createParser({
    onOpenTagName(range) {
      const tag = parser.read(range).toLowerCase();
    ...
    onAttrValue(range) {
      ...
      if (raw.startsWith("{") && raw.endsWith("}")) {
        replacement = transformScriptExpression(raw, options, innerI18nMap);
      }
      // string attribute
      else if (raw.startsWith('"') || raw.startsWith("'")) {
        replacement = transformText(raw.slice(1, -1), options, innerI18nMap);
      }
