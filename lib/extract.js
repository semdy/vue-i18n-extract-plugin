...
const { createI18nVisitor, generateId } = require("./visitors");
const {
  relativeCWDPath,
  shouldExtract,
  trimEmptyLine,
...
function transformText(text, options, innerI18nMap) {
  ...
  if (options.pipeStyle) {
    if (options.keepDefaultMsg) {
      return `'${generateId(text, options)}' | t:'${text}'`;
    }
    return `'${generateId(text, options)}' | t`;
  }
...
function transformMarkoTemplate(code, options, ms) {
...
function transformAngularTemplate(code, options, ms, templateUrl) {
  ...
  function transformExpression(expr, parentSourceSpan) {
    if (!expr) return;
    ...
    switch (type) {
      case "LiteralPrimitive":
        if (typeof expr.value === "string") {
          ...
          if (replacement) {
            ...
            if (parentSourceSpan) {
              ms?.overwrite(
                parentSourceSpan.start.offset,
                parentSourceSpan.end.offset,
                `{{ ${replacement} }}`
              );
            } else {
              ms?.overwrite(
      ...
      case "BindingPipe":
        ...
        if (
          ...
          if (!text.trim() || !_shouldExtract(text, options)) return;
          ...
          if (expr.args?.length > 0) {
            ...
            // 如果第一个参数是对象 {name: 'y'}
            if (firstArg.constructor.name === "LiteralMap") {
              valuesArg = firstArg;
            }
          }
          ...
          let newPipe = `'${id}' | ${options.translateKey}`;
          ...
          if (valuesArg) {
            const rawValues = code.slice(
              valuesArg.sourceSpan.start,
              valuesArg.sourceSpan.end
            );
      ...
      case "Interpolation": // {{ ... }}
        expr.expressions.forEach(it =>
          transformExpression(it, parentSourceSpan)
        );
        break;
...
function processMarkoFile(code, options) {
  // 将input等未自闭合标签替换成闭合标签
  code = code.replace(/<(input|img|br|hr|meta|link)\b([^>]*)>/g, "<$1$2 />");

  const ms = options.rewrite ? new MagicString(code) : null;
  const result = transformMarkoTemplate(code, options, ms);
...
function processAngularFile(code, options, templateUrl) {
  const ms = options.rewrite ? new MagicString(code) : null;
  const result = transformAngularTemplate(code, options, ms, templateUrl);
