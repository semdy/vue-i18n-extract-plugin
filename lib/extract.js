...
function isTCall(expr, translateKey) {
  return (
    expr &&
    expr.type === "CallExpression" &&
    expr.callee?.type === "Identifier" &&
    expr.callee.name === translateKey
  );
}
...
function transformSvelteTemplate(htmlAst, options, ms) {
  ...
  function transformExpression(expr) {
    ...
    switch (expr.type) {
      ...
      case "CallExpression":
        if (isTCall(expr, options.translateKey)) {
...
function transformMarkoTemplate(code, options, ms) {
  ...
  const parser = createParser({
    ...
    onOpenTagEnd() {
      ...
      if (!currentTagInfo.hasId && currentTagInfo.msg) {
        const id = generateId(currentTagInfo.msg, options);
    ...
    onAttrValue(range) {
      ...
      if (currentTagInfo && currentTagInfo.currentAttr === "msg") {
        currentTagInfo.msg = raw.replace(/^["']|["']$/g, "");
        return;
      }
