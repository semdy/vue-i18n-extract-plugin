...
class WebpackPluginI18n {
  ...
  apply(compiler) {
    ...
    const hasCustomLoader = rule =>
      ...
      rule.use.some(
        ({ loader }) => loader && loader.includes("vue-i18n-loader/index.js")
      );
      ...
      if (
        ...
        compiler.options.module?.rules.push({
          ...
          test: generateAdvancedRegex(
            [...allowedExtensions, ...(option.extraFileExtensions || [])],
            this.options
          ),
          // 设置 Loader 执行顺序为后置
          enforce: "post",
          use: [
            {
              // 指定自定义 Loader 的路径
              loader: path.resolve(__dirname, "./vue-i18n-loader/index.js"),
     ...
     * 在构建阶段执行翻译任务
     */
    compiler.hooks.emit.tapPromise("webpackPluginI18n", async _compilation => {
      // 输出构建阶段开始批量翻译的信息
 ...
 */
function generateAdvancedRegex(extensions, option) {
  // 生成扩展名的正则表达式部分
