...
class WebpackPluginI18n {
  ...
  async loadConfig() {
    ...
    try {
      ...
      console.warn(
        `[WebpackPluginI18n] config file failed to load:`,
        error.message
      );
  ...
  loadConfigSync() {
    ...
    try {
      ...
      console.warn(`[WebpackPluginI18n] Failed to load config: ${err.message}`);
      this.options = { ...defaultOptions, ...this.options };
    }
  }
  ...
  apply(compiler) {
    ...
    compiler.hooks.done.tap("webpackPluginI18n", () => {
      // 输出构建阶段开始批量翻译的信息
      if (!this.options.enabled || !this.options.outputJsonFileInPlugin) return;
