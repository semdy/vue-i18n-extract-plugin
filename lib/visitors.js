...
function getTranslateIdentifierName(programPath, option) {
  if (!programPath) {
    return `${option.translateKey}$1`;
  }

  const binding = programPath.scope.getBinding(option.translateKey);

  if (!binding) {
    return `${option.translateKey}$1`;
  }

  return t.identifier(binding.identifier.name).name;
}
  ...
  return {
    ...
    StringLiteral(path) {
      ...
      if (isVNodeCall(parentPath, "_createTextVNode")) {
        ...
        if (!hasCreateVNode) {
          const identifierName = getTranslateIdentifierName(
            programPath,
            option
          );
          // 生成 $t("hashed")
          callExpression = t.callExpression(
            t.identifier(identifierName || option.translateKey),
            [
              t.stringLiteral(generateText(value, hashed, option)),
              option.keepDefaultMsg ? t.stringLiteral(value) : null
            ].filter(Boolean)
    ...
    TemplateElement(path) {
      ...
      const identifierName = getTranslateIdentifierName(programPath, option);

      // 替换为字符类型翻译节点
      const tCallExpression = option.keepDefaultMsg
        ? `${identifierName || option.translateKey}('${generateText(value, hashed, option)}', ${JSON.stringify(value)})`
        : `${identifierName || option.translateKey}('${generateText(value, hashed, option)}')`;
