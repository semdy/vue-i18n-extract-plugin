...
const { babelI18nImportTransform } = require("./babel-plugin-i18n-import");
const {
...
function hasJSXMember(name) {
  return ["jsx", "jsxs", "jsxDEV", "_jsx", "_jsxs", "_jsxDEV"].includes(name);
}
...
function isInJSXRuntimeCall(path) {
  ...
  while (current && current.parentPath) {
    ...
    if (p.isCallExpression()) {
      ...
      if (!callee || !isJSXRuntimeCallee(callee)) {
        current = p;
        continue;
      }
  ...
  return {
    Program(path) {
      if (option.autoImportI18n) {
        const result = babelI18nImportTransform.call(
          this,
          path,
          path.node,
          option
