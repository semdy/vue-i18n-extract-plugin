...
import { EMPTY_PLACEHOLDER, generateId, shouldExtract } from "./core/index.js";
import {
  extractFunctionName,
  hasHtmlTag,
  EXCLUDED_CALL,
...
function createHooksDeclaration(options) {
  const id = options.destructuredHooks
    ? t.objectPattern([
        t.objectProperty(
...
function ensureInjectedHooks(statements, options) {
  for (const statement of statements) {
    ...
    for (const declarator of statement.node.declarations) {
      ...
      // const { xxx } = useTranslation()
      if (t.isObjectPattern(declarator.id)) {
        const hasT = declarator.id.properties.some(property => {
          ...
          return (
            t.isIdentifier(property.key, {
              name: options.translateKey
            }) ||
            t.isStringLiteral(property.key, {
              value: options.translateKey
            })
          );
        });
...
function injectHooks(fnPath, options) {
  ...
  const hasInjectedHooks = ensureInjectedHooks(statements, options);
...
function transformJSXElementCallExpression(node, option, i18nMap) {
  ...
  const msgValue = normalizeText(msgProp?.value.value);

  if (!msgValue && !shouldExtract(msgValue, option)) return;
...
function createI18nVisitor(option, i18nMap) {
  ...
  let importTransformTracker = null;

  return {
    Program: {
      enter(path, state) {
        importTransformTracker = null;

        if (option.autoImportI18n) {
          const canRunImportTransform = shouldRunI18nImportTransform(
            path,
            state
          );

          if (!canRunImportTransform) {
            return;
          }

          importTransformTracker = createI18nImportTransformTracker(option);
        }

        if (state && isVueLike(state.file?.opts?.filename)) {
          hoistedMap.clear();
      ...
      exit(path) {
        importTransformTracker?.apply(path);
        importTransformTracker = null;
      }
    },
    ImportDeclaration(path) {
      importTransformTracker?.trackImportDeclaration(path);
    },
    VariableDeclarator(path) {
      importTransformTracker?.trackVariableDeclarator(path);
    },
    FunctionDeclaration(path) {
      importTransformTracker?.trackFunctionDeclaration(path);
    },
    CallExpression(path) {
      if (path.node.__SKIP_REPLACED_CALL_EXPRESSION) {
        path.skip();
        return;
      }
      ...
      if (!keyText || !shouldExtract(keyText, option)) {
        path.skip();

        // rewrite模式仍记录id，方便后续清理无用翻译
        if (option.rewrite && i18nMap && keyText) {
          i18nMap[keyText] = EMPTY_PLACEHOLDER;
        }
    ...
    StringLiteral(path) {
      ...
      callExpression.__SKIP_REPLACED_CALL_EXPRESSION = true;
    ...
    TemplateElement(path) {
      ...
      const value = normalizeText(node.value.raw || node.value.cooked);
    ...
    JSXElement(path) {
      ...
      if (i18nMap && idValue) {
        if (msgValue) {
          ...
          i18nMap[idValue] = EMPTY_PLACEHOLDER;
        }
      }
  ...
  return () => {
    return {
      visitor: {
        Program(path, state) {
          if (option.autoImportI18n) {
            batchInjectI18nImport(path, option, state);
          }
        }
      }
    };
