...
function isTFunctionByNode(node, option) {
  if (!t.isCallExpression(node)) return false;

  const callee = unwrapCallee(node.callee);

  const is =
    t.isIdentifier(callee, { name: option.translateKey }) || // $t(...)
    (t.isMemberExpression(callee) &&
      t.isIdentifier(callee.property, {
        name: option.translateKey // xxx.$t(...)
      }));
...
function isTFunction(pathOrFnName, option) {
  ...
  if (typeof pathOrFnName === "string") {
    return (
      pathOrFnName === option.translateKey ||
      getLastName(pathOrFnName) === option.translateKey
    );
  }

  return isTFunctionByNode(pathOrFnName.node, option);
}
...
function transformJSXElementCallExpression(node, option, i18nMap) {
  ...
  const msgValue = msgProp.value.value.trim();
  ...
  return {
    Program(path) {
      if (option.autoImportI18n) {
        const result = babelI18nImportTransform.call(this, path, option);
        if (result === false) {
          return;
        }
      }
    ...
    StringLiteral(path) {
      ...
      // 获取调用函数名称
      const fnName = extractFunctionName(callPath);
    ...
    TemplateElement(path) {
      ...
      const value = (node.value.raw || node.value.cooked).trim();
      ...
      if (fnName && excludedCall.includes(fnName)) {
        return;
      }
  ...
  return () => {
    return {
      visitor: {
        Program(path) {
          if (option.autoImportI18n) {
            babelI18nImportTransform.call(this, path, option);
          }
        }
      }
    };
