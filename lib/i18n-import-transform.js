...
function i18nImportTransform(code, path, options) {
  ...
  try {
    ...
    // 只有当需要修改时才重新生成代码
    if (transformNeeded) {
      const { code: newScript } = generate(ast); // { sourceMaps: true, sourceFileName: path }
    ...
    console.warn(`⚠️ [auto-import-i18n] Failed to parse ${path}:`, err);
    return code;
  }
}
